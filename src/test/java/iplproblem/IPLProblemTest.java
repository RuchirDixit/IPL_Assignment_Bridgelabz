...
public class IPLProblemTest {
    ...
    public void givenIPLWicketsData_WhenSortedDescendingOnBowlingAverages_ShouldReturnCorrectPlayerName() {
        try {
            IPLAnalyzer iplAnalyzer = new IPLAnalyzer();
            iplAnalyzer.loadIplData(MOST_WICKETS_CSV_FILE_PATH);
    ...
    //UC13
    @Test
    public void givenIPLWicketsData_WhenSortedDescendingOnMaxBatAndBowlAverage_ShouldReturnCorrectPlayerName() {
        try {
            IPLAnalyzer iplAnalyzer = new IPLAnalyzer();
            iplAnalyzer.loadIplData(MOST_RUNS_CSV_FILE_PATH,MOST_WICKETS_CSV_FILE_PATH);
            String sortedDataSR = iplAnalyzer.getMaximumBattingBowlingAverageWiseSorting();
            MostWicketsCSV[] mostWicketsCSV = new Gson().fromJson(sortedDataSR, MostWicketsCSV[].class);
            Assert.assertEquals("MS Dhoni", mostWicketsCSV[0].playerName);
        } catch(IPLException e) {
        }
    }
    ...
    //UC16
    @Test
    public void givenIPLWicketsData_WhenSortedAscendingHundredFiftyAndAverage_ShouldReturnCorrectPlayerName() {
        try {
            IPLAnalyzer iplAnalyzer = new IPLAnalyzer();
            iplAnalyzer.loadIplData(MOST_RUNS_CSV_FILE_PATH);
            String sortedDataSR = iplAnalyzer.getMaximumHundredFiftyAverageSorting();
            MostRunsCSV[] mostRunsCSV = new Gson().fromJson(sortedDataSR, MostRunsCSV[].class);
            Assert.assertEquals("David Warner ",mostRunsCSV[0].playerName);
        } catch(IPLException e) {
        }
